...
export const ProfileRecurringPaymentSubscriptions = () => import('../index');

describe('ProfileRecurringPaymentSubscriptions renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ProfileRecurringPaymentSubscriptions};
  beforeAll(async () => {
    /**
  ...
  it('ProfileRecurringPaymentSubscriptions works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ProfileRecurringPaymentSubscriptions',
            regions: []
          }
        }
      }
