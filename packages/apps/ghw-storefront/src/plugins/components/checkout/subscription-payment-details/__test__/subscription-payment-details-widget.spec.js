...
export const SubscriptionPaymentDetails = () => import('../index');

describe('SubscriptionPaymentDetails renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {SubscriptionPaymentDetails};
  beforeAll(async () => {
    /**
  ...
  it('SubscriptionPaymentDetails works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'SubscriptionPaymentDetails',
            regions: []
          }
        }
      }
