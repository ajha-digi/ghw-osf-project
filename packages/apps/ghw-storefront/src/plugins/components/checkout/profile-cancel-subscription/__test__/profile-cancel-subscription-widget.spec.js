...
export const ProfileCancelSubscription = () => import('../index');

describe('ProfileCancelSubscription renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ProfileCancelSubscription};
  beforeAll(async () => {
    /**
  ...
  it('ProfileCancelSubscription works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ProfileCancelSubscription',
            regions: []
          }
        }
      }
