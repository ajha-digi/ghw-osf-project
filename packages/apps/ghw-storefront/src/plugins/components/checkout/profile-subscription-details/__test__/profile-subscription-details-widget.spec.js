...
export const ProfileSubscriptionDetails = () => import('../index');

describe('ProfileSubscriptionDetails renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ProfileSubscriptionDetails};
  beforeAll(async () => {
    /**
  ...
  it('ProfileSubscriptionDetails works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ProfileSubscriptionDetails',
            regions: []
          }
        }
      }
