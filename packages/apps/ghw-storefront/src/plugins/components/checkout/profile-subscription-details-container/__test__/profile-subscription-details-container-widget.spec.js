...
export const ProfileSubscriptionDetailsContainer = () => import('../index');

describe('ProfileSubscriptionDetailsContainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ProfileSubscriptionDetailsContainer};
  beforeAll(async () => {
    /**
  ...
  it('ProfileSubscriptionDetailsContainer works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ProfileSubscriptionDetailsContainer',
            regions: []
          }
        }
      }
