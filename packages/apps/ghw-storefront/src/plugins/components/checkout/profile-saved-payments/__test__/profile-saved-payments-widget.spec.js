...
export const ProfileSavedPayments = () => import('../index');

describe('ProfileSavedPayments renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ProfileSavedPayments};
  beforeAll(async () => {
    /**
  ...
  it('ProfileSavedPayments works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ProfileSavedPayments',
            regions: []
          }
        }
      }
