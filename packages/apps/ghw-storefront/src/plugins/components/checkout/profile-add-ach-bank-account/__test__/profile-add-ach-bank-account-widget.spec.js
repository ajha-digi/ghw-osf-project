...
export const ProfileAddACHBankAccount = () => import('../index');

describe('ProfileAddACHBankAccount renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ProfileAddACHBankAccount};
  beforeAll(async () => {
    /**
  ...
  it('ProfileAddACHBankAccount works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ProfileAddACHBankAccount',
            regions: []
          }
        }
      }
