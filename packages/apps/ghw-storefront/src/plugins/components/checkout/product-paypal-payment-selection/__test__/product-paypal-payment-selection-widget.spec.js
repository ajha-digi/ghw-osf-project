...
export const ProductPaypalPaymentSelection = () => import('../index');

describe('ProductPaypalPaymentSelection renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ProductPaypalPaymentSelection};
  beforeAll(async () => {
    /**
  ...
  it('ProductPaypalPaymentSelection works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ProductPaypalPaymentSelection',
            regions: []
          }
        }
      }
