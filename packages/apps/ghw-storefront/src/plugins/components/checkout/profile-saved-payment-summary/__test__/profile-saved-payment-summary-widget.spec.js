...
export const ProfileSavedPaymentSummary = () => import('../index');

describe('ProfileSavedPaymentSummary renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {ProfileSavedPaymentSummary};
  beforeAll(async () => {
    /**
  ...
  it('ProfileSavedPaymentSummary works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'ProfileSavedPaymentSummary',
            regions: []
          }
        }
      }
