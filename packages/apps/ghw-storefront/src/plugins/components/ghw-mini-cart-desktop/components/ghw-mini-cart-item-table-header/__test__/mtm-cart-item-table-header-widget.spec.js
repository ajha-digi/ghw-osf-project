...
export const GHWCartItemTableHeader = () => import('../index');

describe('GHWCartItemTableHeader renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {GHWCartItemTableHeader};
  beforeAll(async () => {
    /**
  ...
  it('GHWCartItemTableHeader works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'GHWCartItemTableHeader',
            regions: []
          }
        }
      }
