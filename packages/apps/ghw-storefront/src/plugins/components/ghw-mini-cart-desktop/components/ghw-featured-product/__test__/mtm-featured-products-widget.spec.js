...
export const GHWFeaturedProducts = () => import('../index');

describe('GHWFeaturedProducts renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {GHWFeaturedProducts};
  beforeAll(async () => {
    /**
  ...
  it('GHWFeaturedProducts works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'GHWFeaturedProducts',
            regions: []
          }
        }
      }
