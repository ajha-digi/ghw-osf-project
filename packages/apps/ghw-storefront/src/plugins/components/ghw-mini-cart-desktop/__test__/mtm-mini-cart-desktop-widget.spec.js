...
export const GHWMiniCartDesktop = () => import('../index');

describe('GHWMiniCartDesktop renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {GHWMiniCartDesktop};
  beforeAll(async () => {
    /**
  ...
  it('GHWMiniCartDesktop works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'GHWMiniCartDesktop',
            regions: []
          }
        }
      }
