...
export const GHWMiniCartDetails = () => import('../index');

describe('GHWMiniCartDetails renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {GHWMiniCartDetails};
  beforeAll(async () => {
    /**
  ...
  it('GHWMiniCartDetails works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'GHWMiniCartDetails',
            regions: []
          }
        }
      }
