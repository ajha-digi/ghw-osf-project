...
export const GHWProductResultImage = () => import('../index');

describe('GHWProductResultImage renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {GHWProductResultImage};
  beforeAll(async () => {
    /**
  ...
  it('GHWProductResultImage works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'GHWProductResultImage',
            regions: []
          }
        }
      }
