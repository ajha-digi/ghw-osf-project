...
function* uspsValidationSaga(action) {
  const response = yield endpointSaga({action, payload: action.payload, endpointId: 'uspsValidation'});

  return response;
}

/**
 * The uspsValidation action.
 *
...
export default {
  *saga() {
    yield takeEvery('uspsValidation', uspsValidationSaga);
  }
};
