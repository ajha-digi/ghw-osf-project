...
function* numberOfOrdersSaga(action) {
  const response = yield endpointSaga({action, payload: action.payload, endpointId: 'numberOfOrders'});

  return response;
}

/**
 * The numberOfOrders action.
 *
...
export default {
  *saga() {
    yield takeEvery('numberOfOrders', numberOfOrdersSaga);
  }
};
