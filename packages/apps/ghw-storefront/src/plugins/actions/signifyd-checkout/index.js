...
function* signifydCheckoutSaga(action) {
  const response = yield endpointSaga({action, payload: action.payload, endpointId: 'signifydCheckout'});

  return response;
}

/**
 * The signifydCheckout action.
 *
...
export default {
  *saga() {
    yield takeEvery('signifydCheckout', signifydCheckoutSaga);
  }
};
