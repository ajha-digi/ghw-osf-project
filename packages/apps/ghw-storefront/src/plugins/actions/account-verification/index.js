...
function* accountVerificationSaga(action) {
  const response = yield endpointSaga({action, payload: action.payload, endpointId: 'accountVerification'});

  return response;
}

/**
 * The accountVerification action.
 *
...
export default {
  *saga() {
    yield takeEvery('accountVerification', accountVerificationSaga);
  }
};
